...
**Result:** Training stopped at epoch 38. Best validation MAE was achieved at epoch 30. Early stopping prevented 8 wasted epochs after the optimal point.
...
|---|---|---|---|---|
| 1 | 2.38 | 2.08 | −0.29 | 0.01158 |
| 5 | 1.65 | 1.52 | −0.14 | 0.00628 |
| 10 | 1.52 | 1.43 | −0.09 | 0.00559 |
| 20 | 1.44 | 1.39 | −0.05 | 0.00525 |
| 30 | 1.39 | 1.33 | −0.06 | 0.00486 |
| 37 | **1.37** | **1.32** ← best (by val MSE) | −0.05 | **0.00473** |
| 40 (final) | 1.36 | 1.34 | −0.02 | 0.00487 |

**Total epochs run:** 40/40 (early stopping did not trigger — val MSE kept setting new bests through epoch 37, only 3 epochs shy of the patience-8 cutoff when training ended)
**Best checkpoint:** epoch 37, selected by lowest **val MSE** (not val MAE — MAE is noisier epoch-to-epoch and can pick a different, less representative epoch; see note below)
**Training time:** approximately 75–115 seconds per epoch on a single NVIDIA A10 GPU
**Total training time:** approximately 60 minutes

> **Note on this rerun:** an earlier version of the training/plotting code had a labeling bug — the column and plots called "MSE" were actually a combined MSE+Pearson-correlation loss (because `--loss combined` was in use), not plain MSE, and the "best epoch" marker was chosen by val MAE instead of val MSE. Both are fixed now; the numbers in this report reflect the corrected rerun. The practical effect on this run was small (best epoch moved from 30 to 37, val MAE from 1.30 to 1.32) — the model was never actually bad, the diagnostics around it were just mislabeled.

### Overfitting Check

| Metric | Old ResNet18 | New SmallResNet |
|---|---|---|
| val/train MSE ratio | **27×** | **1.0×** |
| Final train/val MAE gap | **+7.0 points** | **−0.02 points** |
| Best val MAE (at best-val-MSE epoch) | 8.70 | **1.32** |

A val/train MSE ratio of **1.0×** means the model performs about equally well on validation and training — no memorization. One mild flag from the automated diagnosis: val loss trended slightly upward in the 3 epochs after the best one (epoch 37 → 40), which is the ordinary shape of a curve near its minimum, not a sign of serious overfitting — it's well within what `--patience 8` is designed to tolerate.
...
| **Pearson r** | **0.863** | Strong linear correlation between predictions and true scores. 1.0 would be perfect. |
| **Spearman rho** | **0.863** | The model ranks images in the correct quality order 86.3% of the time. This is the key metric for a quality model. |
| **R²** | **0.738** | The model explains 73.8% of the variance in quality scores across the test set. |

### What these numbers tell us

- **Pearson r = 0.863 and Spearman rho = 0.863** are strong results. Both sit well above 0.8, which is generally considered a good correlation for this type of prediction task. The model reliably identifies which images are higher quality and which are lower quality.
...
|---|---|---|---|
| ffhq_all/49936.png | 23.3 | 24.3 | +1.0 |
| ffhq_all/04591.png | 22.4 | 22.3 | −0.1 |
| ffhq_all/10849.png | 26.0 | 23.1 | −2.9 |
| ffhq_all/17289.png | 17.3 | 20.2 | +2.9 |
| ffhq_all/64087.png | 18.8 | 18.2 | −0.6 |
| ffhq_all/57157.png | 19.2 | 22.6 | +3.3 |
| ffhq_all/50969.png | 21.3 | 21.0 | −0.4 |
| ffhq_all/44661.png | 24.5 | 23.7 | −0.8 |
| ffhq_all/38253.png | 21.1 | 18.6 | −2.5 |
| ffhq_all/31335.png | 25.0 | 24.5 | −0.5 |
| ffhq_all/25036.png | 27.3 | 24.6 | −2.7 |
| ffhq_all/41869.png | 30.1 | 27.4 | −2.6 |
...
| File | What it is |
|---|---|
| `train_curve_full.png` | Training curve plot: true MSE and MAE, train vs val, per epoch. The dashed line marks the best epoch by val MSE (37). Train/val lines track closely — visual evidence of no overfitting. |
| `eval_scatter_full.png` | Scatter plot: predicted score vs true score for all 7,000 test images. A tight diagonal line means good predictions. |
...
| `best_model_full.pt` | The saved model weights (the actual trained model). |
...
2. **Strong correlation** — Pearson r = 0.863, Spearman rho = 0.863. The model reliably identifies image quality differences.
